...
export async function handleButton(interaction: ButtonInteraction) {
  ...
  if (action === "banca_apri") {
    ...
    if (existing) {
      return interaction.reply({
        content: "❌ Hai già un conto in banca!",
        ephemeral: true,
      });
    }
  ...
  if (action === "banca_pin") {
    ...
    if (!account) {
      return interaction.reply({
        content: "❌ Non hai un conto bancario.",
        ephemeral: true,
      });
    }
  ...
  if (action === "banca_carta") {
    ...
    const existing = db
      .prepare("SELECT id FROM cards WHERE userId = ?")
      .get(interaction.user.id);
    if (existing) {
      return interaction.reply({
        content: "❌ Hai già una carta bancaria!",
        ephemeral: true,
      });
    }
  ...
  if (action === "candidatura_open") {
    ...
    if (jobs.length === 0) {
      return interaction.reply({
        content: "❌ Non ci sono lavori disponibili al momento.",
        ephemeral: true,
      });
    }
  ...
  if (action === "job_accept") {
    ...
    if (!job) {
      return interaction.reply({
        content: "❌ Il lavoro non esiste più.",
        ephemeral: true,
      });
    }
          ...
          .send({
            embeds: [
              new EmbedBuilder()
                ...
                .setDescription(
                  `La tua candidatura per **${job.name}** è stata **accettata**!`,
                )
                .setColor(0x57f287)
                .setTimestamp(),
            ],
          ...
          .send({
            embeds: [
              new EmbedBuilder()
                ...
                .setDescription(
                  `La tua candidatura per **${job?.name ?? "il lavoro"}** è stata **rifiutata**.`,
                )
                .setColor(0xed4245)
                .setTimestamp(),
            ],
  ...
  if (action === "negozio_open") {
    ...
    if (shops.length === 0) {
      return interaction.reply({
        content: "❌ Nessun negozio disponibile al momento.",
        ephemeral: true,
      });
    }
    const select = new StringSelectMenuBuilder()
  ...
  if (
    action === "negozio_richiesta_approva" ||
    action === "negozio_richiesta_rifiuta"
  ) {
    const guild =
      interaction.guild ??
    ...
    if (
      !guild ||
      !memberIsStaff(interaction.member, interaction.user.id, guild.ownerId)
    ) {
      return interaction.reply({
        content: "❌ Solo lo staff può gestire questa richiesta.",
        ephemeral: true,
      });
    }
    ...
    const request = db
      .prepare("SELECT * FROM shop_requests WHERE id = ?")
      .get(requestId) as ShopRequest | undefined;
    if (!request || request.status !== "pending") {
      return interaction.reply({
        content: "❌ Richiesta non trovata o già gestita.",
        ephemeral: true,
      });
    }
    ...
    if (action === "negozio_richiesta_approva") {
      const outcome = db.transaction(() => {
        ...
        if (dup) {
          db.prepare(
            "UPDATE shop_requests SET status = 'rejected' WHERE id = ?",
          ).run(requestId);
          return "duplicate" as const;
        }

        db.prepare("INSERT INTO shops (name, ownerId) VALUES (?, ?)").run(
          fresh.shopName,
          fresh.userId,
        );
        db.prepare(
          "UPDATE shop_requests SET status = 'approved' WHERE id = ?",
        ).run(requestId);
      ...
      if (outcome === "already_handled") {
        return interaction.reply({
          content: "❌ Richiesta già gestita da un altro membro dello staff.",
          ephemeral: true,
        });
      }
          ...
          .send({
            embeds: [
              new EmbedBuilder()
                ...
                .setDescription(
                  `Il tuo negozio **${request.shopName}** è stato approvato ed è ora attivo!`,
                )
                .setColor(0x57f287)
                .setTimestamp(),
            ],
          ...
          .send({
            embeds: [
              new EmbedBuilder()
                ...
                .setDescription(
                  `La tua richiesta per aprire il negozio **${request.shopName}** è stata rifiutata.`,
                )
                .setColor(0xed4245)
                .setTimestamp(),
            ],
  ...
  if (action === "eliminaprodotto_open") {
    ...
    if (!isEmployee) {
      return interaction.reply({
        content:
          "❌ Solo i dipendenti (con un lavoro assegnato) possono eliminare prodotti.",
        ephemeral: true,
      });
    }
    ...
    return interaction.reply({
      content: "Seleziona il negozio da cui eliminare un prodotto:",
      components: [row],
      ephemeral: true,
    });
  }
  ...
  if (action === "dimissioni_open") {
    ...
    `,
      ...
      | { userId: string; jobId: number; jobName: string; roleId: string }
      | undefined;

    if (!emp) {
      return interaction.reply({
        content: "❌ Non sei assunto in nessun lavoro.",
        ephemeral: true,
      });
    }
  ...
  if (action === "dimissioni_confirm") {
    ...
    if (!emp) {
      return interaction.update({
        content: "❌ Non risulti più assunto in questo lavoro.",
        components: [],
        embeds: [],
      });
    }

    db.prepare("DELETE FROM employees WHERE userId = ? AND jobId = ?").run(
      userId,
      jobId,
    );
    db.prepare(
      "UPDATE jobs SET currentSlots = MAX(0, currentSlots - 1) WHERE id = ?",
    ).run(jobId);
    ...
    if (guild) {
      try {
        ...
        /* ignora se il ruolo non esiste più */
      }
    }
  ...
  if (action === "licenziamento_open") {
    ...
    if (
      ...
      return interaction.reply({
        content: "❌ Non hai i permessi per licenziare dipendenti.",
        ephemeral: true,
      });
    }
    ...
    if (employees.length === 0) {
      return interaction.reply({
        content: "❌ Non ci sono dipendenti al momento.",
        ephemeral: true,
      });
    }
    ...
    for (const e of employees.slice(0, 25)) {
      try {
        ...
        const opt = options.find(
          (o) => o.data.value === `${e.userId}:${e.jobId}`,
        );
        if (member && opt) {
  ...
  if (action === "auto_buy") {
    ...
    const car = db.prepare("SELECT * FROM cars WHERE id = ?").get(carId) as
      | { id: number; name: string; price: number }
      | undefined;
    if (!car) {
      return interaction.reply({
        content: "❌ Auto non trovata.",
        ephemeral: true,
      });
    }
    if (account.balance < car.price) {
      return interaction.reply({
        content: `❌ Saldo insufficiente. Ti servono **€${car.price}**, hai **€${account.balance}**.`,
        ephemeral: true,
      });
    }
    db.prepare(
      "UPDATE accounts SET balance = balance - ? WHERE userId = ?",
    ).run(car.price, interaction.user.id);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "Nuovo Saldo",
          value: `€${account.balance - car.price}`,
          inline: true,
        },
      )
      .setTimestamp();
  ...
  if (action === "house_buy") {
    ...
    if (account.balance < house.price) {
      return interaction.reply({
        content: `❌ Saldo insufficiente. Ti servono **€${house.price}**, hai **€${account.balance}**.`,
        ephemeral: true,
      });
    }
    db.prepare(
      "UPDATE accounts SET balance = balance - ? WHERE userId = ?",
    ).run(house.price, interaction.user.id);
  ...
  if (action === "product_buy") {
    ...
    const product = db
      ...
      .get(parseInt(productId)) as Product | undefined;
    if (!product) {
      return interaction.reply({
        content: "❌ Prodotto non trovato.",
        ephemeral: true,
      });
    }
    const modal = new ModalBuilder()
...
export async function handleSelectMenu(
  ...
  if (action === "candidatura_select") {
    ...
    const alreadyEmp = db
      .prepare("SELECT userId FROM employees WHERE userId = ? AND jobId = ?")
      .get(userId, jobId);
    if (alreadyEmp) {
      return interaction.reply({
        content: `❌ Sei già assunto come **${job.name}**.`,
        ephemeral: true,
      });
    }
    ...
    if (pendingApp) {
      return interaction.reply({
        content: "❌ Hai già una candidatura in attesa per questo lavoro.",
        ephemeral: true,
      });
    }
    ...
    if (bandoChannelId) {
      ...
      if (job.maxSlots !== null && job.currentSlots >= job.maxSlots) {
        return interaction.reply({
          content: `❌ Non ci sono posti disponibili per **${job.name}**.`,
          ephemeral: true,
        });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: "Stipendio", value: `€${job.salary}`, inline: true },
          {
            name: "Posti rimanenti",
            value: job.maxSlots
              ? `${job.maxSlots - job.currentSlots}`
              : "Illimitati",
            inline: true,
          },
          { name: "Server", value: guild.name, inline: true },
      ...
      try {
        ...
        const staffMembers = guild.members.cache.filter(
          (m) => m.roles.cache.has(STAFF_ROLE_ID) && m.id !== guild.ownerId,
        );
      ...
      if (reached === 0) {
        ...
        return interaction.reply({
          content:
            "❌ Impossibile contattare il proprietario o lo staff (DM chiusi). Contattali direttamente.",
          ephemeral: true,
        });
      }
  ...
  if (action === "creaprodotto_select_shop") {
    ...
    const shop = db.prepare("SELECT * FROM shops WHERE id = ?").get(shopId) as
      | Shop
      | undefined;
    if (!shop) {
      return interaction.reply({
        content: "❌ Negozio non trovato.",
        ephemeral: true,
      });
    }
  ...
  if (action === "eliminaprodotto_select_shop") {
    ...
    const products = db
      .prepare(
        "SELECT * FROM products WHERE shopId = ? AND createdBy = ? ORDER BY name ASC",
      )
      .all(shopId, interaction.user.id) as Product[];
    if (products.length === 0) {
      return interaction.reply({
        content: `❌ Non hai creato nessun prodotto nel negozio **${shop.name}**.`,
        ephemeral: true,
      });
    }
    ...
    return interaction.reply({
      content: `Seleziona il prodotto da eliminare da **${shop.name}**:`,
      components: [row],
      ephemeral: true,
    });
  }
  ...
  if (action === "eliminaprodotto_select_product") {
    ...
    if (!product) {
      return interaction.reply({
        content: "❌ Prodotto non trovato (forse già eliminato).",
        ephemeral: true,
      });
    }
    if (product.createdBy !== interaction.user.id) {
      return interaction.reply({
        content: "❌ Non puoi eliminare un prodotto che non hai creato tu.",
        ephemeral: true,
      });
    }
  ...
  if (action === "negozio_select") {
    ...
    const products = db
      .prepare("SELECT * FROM products WHERE shopId = ? ORDER BY price ASC")
      .all(shopId) as Product[];
    if (products.length === 0) {
      return interaction.reply({
        content: `❌ Il negozio **${shop.name}** non ha prodotti al momento.`,
        ephemeral: true,
      });
    }
...
export async function handleModal(interaction: ModalSubmitInteraction) {
  ...
  if (action === "negozio_richiedi_modal") {
    ...
    const shopDesc =
      interaction.fields.getTextInputValue("shop_desc")?.trim() ||
    ...
    if (existing) {
      return interaction.reply({
        content: `❌ Esiste già un negozio chiamato "${shopName}".`,
        ephemeral: true,
      });
    }
    ...
    const channel = await guild.channels
      .create({
        name: `richiesta-negozio-${interaction.user.username}`
          .toLowerCase()
          .slice(0, 90),
    ...
    if (!channel) {
      return interaction.editReply({
        content:
          "❌ Impossibile creare il canale della richiesta. Contatta lo staff.",
      });
    }
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
        .setCustomId(`negozio_richiesta_approva:${requestId}`)
        .setLabel("✅ Approva")
        .setStyle(ButtonStyle.Success),
      new ButtonBuilder()
        .setCustomId(`negozio_richiesta_rifiuta:${requestId}`)
        .setLabel("❌ Rifiuta")
        .setStyle(ButtonStyle.Danger),
    );
    ...
    return interaction.editReply({
      content: `✅ Richiesta inviata! Discutine con lo staff nel canale ${channel}.`,
    });
  }
  ...
  if (action === "creaprodotto_modal") {
    ...
    const descrizione =
      interaction.fields.getTextInputValue("descrizione")?.trim() || "";

    const prezzo = parseInt(prezzoStr);
    if (isNaN(prezzo) || prezzo < 0) {
      return interaction.reply({
        content: "❌ Prezzo non valido. Inserisci solo numeri.",
        ephemeral: true,
      });
    }
  ...
  if (action === "product_buy_modal") {
    ...
    `,
      ...
      .get(productId) as
      | (Product & { shopName: string; shopOwnerId: string | null })
      | undefined;
    if (!product) {
      return interaction.reply({
        content: "❌ Prodotto non trovato.",
    ...
    if (debit.changes === 0) {
      const fresh = db
        .prepare("SELECT * FROM accounts WHERE userId = ?")
        .get(interaction.user.id) as Account;
      return interaction.reply({
        content: `❌ Saldo insufficiente. Ti servono **€${product.price}**, hai **€${fresh.balance}**.`,
        ephemeral: true,
      });
    }
    ...
    const newBalance = (
      db
        .prepare("SELECT balance FROM accounts WHERE userId = ?")
        .get(interaction.user.id) as { balance: number }
    ).balance;
            ...
            embeds: [
              new EmbedBuilder()
                ...
                .addFields(
                  ...
                  {
                    name: "Username Roblox acquirente",
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `Hai acquistato **${product.name}** dal negozio **${product.shopName}**.\n\n📅 Presentati al prossimo RP per ritirare il pacco!`,
              )
              .setColor(0x5865f2)
              .addFields(
                { name: "Prodotto", value: product.name, inline: true },
    ...
    try {
      const postinoRole = await guild.roles
        .fetch(POSTINO_ROLE_ID)
        .catch(() => null);
      if (postinoRole) {
        await guild.members.fetch();
        const postinoEmbed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "Prodotto da consegnare",
              value: product.name,
              inline: true,
            },
            { name: "Negozio", value: product.shopName, inline: true },
            {
              name: "Consegna a (Roblox)",
              value: robloxUsername,
              inline: true,
            },
            {
              name: "Acquirente Discord",
              value: `<@${interaction.user.id}>`,
              inline: true,
            },
          )
  ...
  if (action === "banca_pin_modal") {
    ...
    if (!/^\d{4}$/.test(pin)) {
      return interaction.reply({
        content: "❌ Il PIN deve essere composto da 4 cifre numeriche.",
        ephemeral: true,
      });
    }
    const account = db
    ...
    db.prepare("UPDATE accounts SET pin = ? WHERE userId = ?").run(
      pin,
      interaction.user.id,
    );
    return interaction.reply({
      content: "✅ PIN impostato con successo!",
      ephemeral: true,
    });
  }

  if (action === "pagamento_modal") {
    const recipientId = interaction.fields
      .getTextInputValue("recipient_id")
      .trim();
    const amountStr = interaction.fields.getTextInputValue("amount").trim();
    const causale =
      interaction.fields.getTextInputValue("causale").trim() ||
      "Nessuna causale";
    ...
    const recipientAccount = db
      .prepare("SELECT * FROM accounts WHERE userId = ?")
      .get(recipientId) as Account | undefined;
    if (!recipientAccount) {
      return interaction.reply({
        content: "❌ Il destinatario non ha un conto bancario.",
        ephemeral: true,
      });
    }
    if (recipientId === interaction.user.id) {
      return interaction.reply({
        content: "❌ Non puoi inviare denaro a te stesso.",
        ephemeral: true,
      });
    }
    ...
    db.prepare(
      "UPDATE accounts SET balance = balance - ? WHERE userId = ?",
    ).run(amount, interaction.user.id);
    db.prepare(
      "UPDATE accounts SET balance = balance + ? WHERE userId = ?",
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "Nuovo Saldo",
          value: `€${senderAccount.balance - amount}`,
          inline: true,
        },
      )
      .setTimestamp();
